...
/**
 * Hook that manages a controlled value with debounced updates and bidirectional synchronization.
 *
 * This hook is useful for controlled inputs where you want to debounce user input before
 * updating the parent state, while also syncing the internal value when the controlled
 * value changes externally. It prevents race conditions by ignoring controlled value
 * changes that originated from debounced updates.
 *
 * @param options - Configuration options
 * @param options.controlledValue - The controlled value from parent state
 * @param options.setControlledValue - Callback to update the parent controlled value
 ...
 * @param options.readonly - Whether the input is readonly (prevents updates to controlled value)
 * @param options.setInputValueExtraDeps - Additional dependencies that trigger internal value sync
 * @param options.checkIfEmpty - Function to determine if value is empty (default: `!value`)
 * @param options.onUnMount - Whether to flush or cancel pending debounced updates on unmount (default: 'flush')
 * @returns Object with defaultValue, isEmpty, onChange, isTouched ref, and flushDebounce function
 *
 * @example
 * ```tsx
