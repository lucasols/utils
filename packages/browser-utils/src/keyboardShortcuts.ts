...
export function ignoreInputTypingEvents(
  ...
  return (e: KeyboardEvent) => {
    ...
    if (!enteringText) {
      e.preventDefault();
      return typeof callback === 'function' ? callback(e) : callback.current(e);
    }
  };
}

export function ignoreRichTextInputTypingEvents(
  callback: Callback | { current: Callback },
) {
  return (e: KeyboardEvent) => {
    const active = document.activeElement;
